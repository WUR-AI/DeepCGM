...
if __name__ == "__main__":
    ...
    # Adjusting the rows to match the selected variables
    nrows = 2
    ncols = 3
    fig, axs = plt.subplots(dpi=300, nrows=nrows, ncols=ncols, figsize=(10, 4))
    
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.8, top=0.9, wspace=0.1, hspace=0.1)
    
    # Max values for 'WSO' and 'WAGT' only
    max_values = [8000, 14000]  # These are for 'WSO' and 'WAGT' respectively
    selected_obs = ['WSO', 'WAGT']  # Selected variables for display
    selected_units = ['kg/ha', 'kg/ha']  # Corresponding units
    selected_indices = [obs_name.index(obs) for obs in selected_obs]
    ...
    for i, obs_index in enumerate(selected_indices):
        for j in range(ncols):
            axs_ij = axs[i, j]
    
            day = wea_fer_group[j][sample_loc, :, 0]
            res = res_group[j][sample_loc, :, obs_index]
            obs = obs_group[j][sample_loc, :, obs_index]
            lstm_pre = pre_list_group[j][0][sample_loc, :, obs_index]
            deepcgm_pre = pre_list_group[j][1][sample_loc, :, obs_index]
            ...
            axs_ij.set_ylim(top=max_values[i])        
    
            if j == 0:
                axs_ij.set_ylabel(f"{selected_obs[i]} (kg/ha)")
            else:
            ...
            axs_ij.text(0.03, 0.85, "(%s%d)" % (chr(97 + i), j+1), transform=axs_ij.transAxes, fontsize=10)
    ...
    for ax, col, j in zip(axs[0], col_titles, range(ncols)):
        ...
        box_height = 0.17  # Height of the gray box
    ...
    legend_handles = [
        ...
        Line2D([0], [0], color='gray', lw=1, label='ORYZA2000'),        # Gray line
        Line2D([0], [0], color='blue', lw=1, label='LSTM'),   # Blue line
        Line2D([0], [0], color="red", lw=1, label='DeepCGM'),    # Red line
    ]
    plt.subplots_adjust(bottom=0.17)
    fig.legend(handles=legend_handles, loc='lower center', ncol=4, frameon=False)
    
    # Save and display
    plt.savefig('figure/Fig.7 Result examples of WSO and WAGT.svg', bbox_inches='tight', format="svg")
    plt.show()
    plt.close()
